...
int* saddle(int* a, int n, int m, int* b);
int pain(int* a, int n, int m);
void main()
{
	...
	while (r!=0)
	{
		...
		printf("|0. Для выхода из программы                                 |\n");
		printf("|                                                           |\n");
		...
		switch (r)
		{
		 case 1:
			 ...
			 printf("\n");
			 printf("\n     { x+1, при х>0.75        ");
			 printf("\nV(x)={ 1-x^5, при 0<=х<=0.75 (2)");
			 printf("\n     {x+ln|cos(x)|, при х<0");
			 printf("\nДля расчёта первой функции нажмите 1, для второй - 2.\n");
			 scanf("%d", &r);
			 switch (r)
			 {
			  case 1:
		 ...
		 case 3:
			 ...
			 a = zip(a, n, m);
			 zip2(a, n, m);
			 printf("Количество отрицательных элементов в тех строках, которые содержат хотя бы один нулевой элемент: %d", pain(a, n, m));
			 saddle(a,n,m,b);
			 printf("\nСедловая точка вашей матрицы = %d(%d;%d)", b[0], b[1], b[2]);
			 break;
		 default:
			 printf("Ошибка!");
			 break;
